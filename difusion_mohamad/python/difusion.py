...
for kk in range(pasos):
	...
	for j in range(lattice_y):
		...
		f[j][lattice_x-1][3]=-f[j][lattice_x-1][1] #CF lado Este
		f[j][lattice_x-1][6]=-f[j][lattice_x-1][8]
		f[j][lattice_x-1][7]=-f[j][lattice_x-1][5]
	
	for j in range(lattice_x):
		f[0][j][4]=-f[0][j][2]	# CF lado Norte
		f[0][j][7]=-f[0][j][5]
		f[0][j][8]=-f[0][j][6]
...
T = f.sum(axis=2)
print(T)
savetxt('Temperatura.txt', T,fmt='%.4f')
