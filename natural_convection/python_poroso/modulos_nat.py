...
#campo de flujo..........................................................
@jit
def colisionf(u,v,f,feq,rho,omega,w,cx,cy,n,m,th,gbeta,TF):
    feq=zeros([9,n,m])
    tref=0.50
    for i in range(n):
        for j in range(m):
...
#                if(j==0 or j==m): force =0.0
                ...
                f[k,i,j]=omega*feq[k,i,j]+(1.-omega)*f[k,i,j]+TF[k,i,j]
    return f
...
@jit
def calc_rho_u_v(f,rho,u,v,cx,cy,n,m,w, gbeta,th,visco):
    tref=0.50
    Da= 1.37e-5
    K = Da*n**2  # Permeabilidad = Darcy * longitud característica^2
    poros = 0.385	#Porosidad
    Fporo=1.75/(sqrt(150*poros**3)) #Función geométrica del poro
    rho = f.sum(axis=0)   # cálculo de la densidad
    TF=zeros([9,n,m])
    G=zeros([9])
    for i in range(n):
        for j in range(m):
            ...
            for k in range(9):
                G[k]=gbeta*(th[i,j]-tref)*cy[k]
                TF[k,i,j] = -w[k]*((poros*visco/K)*(u[i,j]*cx[k] + v[i,j]*cy[k]) + \
                (poros*Fporo/sqrt(K))*(abs(u[i,j])*u[i,j]*cx[k] + abs(v[i,j])*v[i,j]*cy[k])) + G[k]
...
#
        g[3,i,m-1] = -g[1,i,m-1]             #condición de frontera este
        g[6,i,m-1] = -g[8,i,m-1]
        g[7,i,m-1] = -g[5,i,m-1]        
