...
do j=0,m
	do i=0,n
		do k=0,8
			f(k,i,j)=w(k)*rho(i,j)  !comienza calculando la función de distribución
...
do kk=1,mstep
	do j=0,m
		do i=0,n
			...
			rho(i,j)=sum !Cominza calculando la temperatura de cada lattice
		end do
	end do
	print *,rho(0,m/2)
	...
	do j=0,m
		do i=0,n
			do k=0,8 
				feq=w(k)*rho(i,j)  !Se aplica la colisión
...
do j=0,m  
	do i=0,n
		sum=0.0 ! Se recalcula la temperatura
		do k=0,8
			sum=sum+f(k,i,j)
		end do
		rho(i,j)=sum
